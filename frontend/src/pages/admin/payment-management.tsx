...
const StatusBadge = ({ status }: {status: string}) => {
  ...
  const config = statusConfig[status as PaymentStatusEnum] || { label: status, className: '' };
...
const AdminPaymentManagement = () => {
  ...
  const queryParams = useMemo<PaymentQueryParams>(() => {
    ...
    if (sortField) params.sort = `${sortOrder === 'desc' ? '-' : ''}${sortField}`;
    
    return params;
  }, [page, limit, statusFilter, methodFilter, searchQuery, startDate, endDate, sortField, sortOrder]);
  ...
  useEffect(() => {
    setPage(1);
  }, [statusFilter, methodFilter, searchQuery, startDate, endDate, sortField, sortOrder]);
  ...
  const filteredPayments = useMemo(() => {
    ...
    return payments.filter(payment => {
      if (currentTab === 'pending' && payment.status !== PaymentStatusEnum.PENDING) return false;
      if (currentTab === 'completed' && payment.status !== PaymentStatusEnum.COMPLETED) return false;
      return true;
    });
  }, [payments, currentTab]);
    ...
    <>
      ...
      <Card className="border-0 shadow-sm">
        <CardHeader>
          ...
          <CardDescription>Quản lý tất cả thanh toán trong hệ thống</CardDescription>
        </CardHeader>
            ...
            {pagination && !isLoading && filteredPayments.length > 0 && (
              <div className="flex justify-between items-center mt-4">
                <div className="text-sm text-muted-foreground text-nowrap">
                  Hiển thị {filteredPayments.length} / {pagination.total} thanh toán
                </div>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="sm:max-w-md">
          ...
          {selectedPayment && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-2 text-sm">
                ...
                <div>
                  {formatDateString(selectedPayment.appointmentId.scheduledDate)}
                </div>
          ...
          <DialogFooter className="flex sm:justify-between">
            <Button variant="outline" onClick={() => setDialogOpen(false)} className="border-0">
              Hủy
            </Button>
            <Button
