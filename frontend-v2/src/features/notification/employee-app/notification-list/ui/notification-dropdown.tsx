...
import { useEmployeeCountUnread } from '@/features/notification/api/get-count-unread';
import { useEmployeeMarkAllAsRead } from '@/features/notification/api/react-all-notification';
...
import { useEmployeeNotificationListController } from '@/features/notification/employee-app/notification-list/application/use-notification-list-controller';
...
export const EmployeeNotificationDropdown = () => {
  const { data, fetchNextPage, hasNextPage, isFetchingNextPage } =
    useEmployeeNotificationListController();
  const scrollRef = useRef<HTMLDivElement>(null);

  const countUnreadQuery = useEmployeeCountUnread();
  const unreadCount = countUnreadQuery.data ?? 0;

  const markAllAsRead = useEmployeeMarkAllAsRead();
...
export const NotificationItem = ({ notification }: { notification: Notification }) => {
  ...
  const handleClick = () => {
    switch (notification.type) {
      case NOTIFICATION_TYPES.NEW_BOOKING_ASSIGNED:
        if (!data) return;
        navigate(paths.employee.bookings.path);
        break;
    }
  };
